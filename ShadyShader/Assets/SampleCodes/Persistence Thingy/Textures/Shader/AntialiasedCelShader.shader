Shader "Custom/Cel/AntialiasedCelShader" 
{
	// PLOP: http://prideout.net/blog/?p=22
	Properties 
	{
		_MainTex("Main Tex", 2D) = "white" {}
		_Color("Base object color", Color) = (1,1,1,1) // Diffuse Color
		_Outline("Outline", Range(0,1)) = 0.1
		_OutlineColor("Outline Color", Color) = (0,0,0,0)
		_SpecularColor("Specular/Light bleaks Color", Color) = (1,1,1,1)
		_Shininess("Shininess", Range(1, 500)) = 40
		//_DiffuseSegment("Diffuse Segment", Vector) = (0.1, 0.3, 0.6, 1.0)
		_SpecularSegment("Specular Gradation (0 super shiny / 1 not shiny", Range(0, 1)) = 0.5
	}
	SubShader 
	{
		...
		pass
		{
			//Forward Rendering path renders each object in one or more passes, depending on 
			// lights that affect the object. Lights themselves are also treated differently by 
			// Forward Rendering, depending on their settings and intensity.
			...
			// multi_compile_fwdbase compiles all variants needed by ForwardBase 
			// (forward rendering base) pass type. The variants deal with different 
			// lightmap types and main directional light having shadows on or off.
			...
			{ 
				_DiffuseSegment = (0.1, 0.3, 0.6, 1.0);
				...
				fixed3 diffuse = diff * _LightColor0.rgb * _Color.rgb * texColor;
